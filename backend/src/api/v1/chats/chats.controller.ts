   ...
   */
	...
	public create() {
		return async (req: Request, res: Response, next?: NextFunction): Promise<Response> => {
			try {
				const resource = await new ChatModel(req.body).save();

				// Sending a broadcast message to all clients
				const socketService = DIContainer.get(SocketsService);
				socketService.broadcast(`newChat`, resource);
				return res.status(CREATED).json(resource);
			} catch (e) {
				next(e);
			}
		};
	...
	public getChatMessage(id?: string, mid?: string) {
		return async (req: Request, res: Response, next?: NextFunction): Promise<Response> => {
			try {
				...
				const resource = await ChatModel.findOne(
					{ _id: modelId },
					{ messages: { $elemMatch: { _id: Types.ObjectId(submodelId) } } }
				)
					.orFail(new NotFound())
					.exec();
				// resource.messages[0]

				return res.status(OK).json(resource.messages[0]);
			} catch (e) {
				next(e);
			}
		};
	...
	public getAllChatMessages(id?: string) {
		return async (req: Request, res: Response, next?: NextFunction): Promise<Response> => {
			try {
				...
				return res.status(OK).json(resource);
			} catch (e) {
				next(e);
			}
		};
	...
	public pushMessageAsReply(id?: string, mid?: string) {
		return async (req: Request, res: Response, next?: NextFunction): Promise<Response> => {
			try {
				...
				const resourceChat = await ChatModel.findOne(
					{ _id: modelId },
					{ messages: { $elemMatch: { _id: Types.ObjectId(submodelId) } } }
				);
				...
				// Sending a broadcast message to all clients
				const socketService = DIContainer.get(SocketsService);
				socketService.broadcast(`/${modelId}/newMessageAsReply`, { message, chatId: modelId });

				return res.status(OK).json(resourceChat.messages[0].replies);
			} catch (e) {
				next(e);
			}
		};
	...
	public pushMessageToChat(id?: string) {
		return async (req: Request, res: Response, next?: NextFunction): Promise<Response> => {
			try {
				...
				// Sending a broadcast message to all clients
				const socketService = DIContainer.get(SocketsService);
				socketService.broadcast(`/${modelId}/newMessage`, { message, chatId: modelId });
